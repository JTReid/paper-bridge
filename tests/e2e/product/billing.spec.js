...
test('inactive account is limited to billing and checkout form uses full-page navigation', async ({ page, family }) => {
...
test('active account can use product and billing portal form uses full-page navigation', async ({ page, family }) => {
  setAccountSubscription(family.accountName, ACTIVE_SUBSCRIPTION);
...
test('trial billing shows the future allowance price and removes the payment promise after cancellation', async ({ page, family }) => {
  ...
  setAccountSubscription(family.accountName, trialSubscription);
  ...
  setAccountSubscription(family.accountName, { ...trialSubscription, cancel_at_period_end: true });
  await page.reload();
...
test('profile allowance blocks additions, permits purchased extras, and preserves profiles after a reduction', async ({ page, family }) => {
  test.setTimeout(60_000);
  const lastName = `Billing Allowance ${randomUUID()}`;

  setAccountSubscription(family.accountName, { ...ACTIVE_SUBSCRIPTION, profile_limit: 5 });
  createAccountProfiles(family.accountName, [
  ...
  setAccountSubscription(family.accountName, { ...ACTIVE_SUBSCRIPTION, profile_limit: 6 });
  await page.goto('/profiles/new');
...
test('successful checkout waits safely and Turbo opens the dashboard after activation', async ({ page, family }) => {
  setAccountSubscription(family.accountName, PENDING_SUBSCRIPTION);
  await signInWithoutDashboardExpectation(page, family.user);
...
test('non-active checkout result returns to billing after the webhook refresh', async ({ page, family }) => {
  setAccountSubscription(family.accountName, PENDING_SUBSCRIPTION);
  await signInWithoutDashboardExpectation(page, family.user);
  await page.goto('/dashboard?checkout=success');
  await expect(page.getByTestId('checkout-pending-page')).toBeVisible();
...
test('canceled checkout returns to billing with a clear notice', async ({ page, family }) => {
  setAccountSubscription(family.accountName, PENDING_SUBSCRIPTION);
  await signInWithoutDashboardExpectation(page, family.user);
