...
| `profiles` | Creates and edits a profile with separate names, keeps grade/school off both forms, verifies edited date of birth and notes, verifies cancellation and confirmation of deletion, and verifies document-backed deletion refusal with a link to the documents. |
...
| `documents` | Exercises original-filename search, category filtering, file-only upload with All Files, selection removal/clearing, successful 51-file upload, storage-only Word/ZIP downloads and editing, consistent single/batch destinations, pending metadata completion and edit unlock, preserved corrections, and original-file actions. Background status/category updates refresh rows and counts while preserving filters, selections, focus, unfinished search text, and open dialogs, including reconnect recovery. Retrying a failed document uses the saved original and runs the real text job through a fake provider connection, checking preserved saved research/history, the existing summary while queued, its removal at worker start, and the new summary before search becomes ready. Uses captured Turbo signals without live AI. |
...
| `calendar` | Creates and emails appointments in the full-page calendar and profile panel. Covers modal editing, canceling, validation retention, profile reassignment, month changes, native keyboard year entry, and confirmed deletion with refreshed counts/grid/agenda. Includes phone editing/deletion while preserving an unfinished Ask question, account calendar navigation, Central Time rendering, and unfinished profile edits beneath the panel. |
...
| `documents` | Verifies required-file and blank-title validation, invalid supported-image rejection, partial-upload notices, duplicate rejection without overwrites, changed contents under the same filename, and identical files across profiles. Successful uploads, including batches larger than 50 files, metadata edits, full retries, and live list updates remain `workflow documents`; live providers and OCR remain outside browser coverage. |
