...
| `documents` | Exercises original-filename search, category filtering, file-only upload with All Files, selection removal/clearing, consistent single/batch destinations, pending metadata completion and edit unlock, preserved corrections, and prominent original-file actions. Uses deterministic completion without live AI. |
...
| `documents` | Verifies required-file and blank-title validation, unsupported-file rejection, and partial-upload notices. Successful uploads and metadata edits remain `workflow documents`; live ingestion, OCR, and embeddings remain outside browser coverage. |
